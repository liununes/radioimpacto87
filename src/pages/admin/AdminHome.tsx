...
const AdminHome = () => {
  ...
  return (
    <div className="space-y-12 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-1000">
        ...
        {stats.map((s, idx) => (
          <Card key={s.label} className="group overflow-hidden rounded-none border-none shadow-xl hover:shadow-2xl transition-all duration-500 cursor-default bg-white text-slate-900">
            <CardContent className="p-8 flex items-center justify-between">
              ...
              <div className={`p-4 rounded-none ${idx % 2 === 0 ? 'bg-primary/5' : 'bg-accent/5'} group-hover:rotate-12 transition-all duration-500`}>
      ...
      <div className="bg-primary rounded-none p-12 text-white relative overflow-hidden shadow-2xl">
         <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
            ...
            <Link to="/admin/aparencia" className="bg-white text-slate-900 text-primary h-16 flex items-center px-10 rounded-none font-black uppercase text-xs tracking-widest shadow-xl hover:scale-105 active:scale-95 transition-all shrink-0">
               Ir para Aparência
            </Link>
         </div>
      </div>
