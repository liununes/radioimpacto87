  ...
  return (
    <div className="max-w-[1200px] mx-auto space-y-10 pb-40 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-card text-card-foreground p-6 rounded-xl border border-border shadow-sm">
        <div>
          <div className="inline-flex items-center gap-2 px-2.5 py-1 bg-primary/10 text-primary rounded-lg text-xs font-semibold mb-2">
            <CheckCircle2 className="w-4 h-4" /> Sistema de Design Ativo
          </div>
          <h2 className="text-2xl font-bold tracking-tight">
            Controle Geral do Site
          </h2>
          <p className="text-sm font-medium text-muted-foreground mt-1">Personalize os dados da rádio, cores e textos.</p>
        </div>
        <Button onClick={handleSave} className="rounded-lg font-semibold text-sm h-11 px-8 bg-primary text-primary-foreground shadow-sm w-full sm:w-auto" disabled={loading}>
          <Save className="w-4 h-4 mr-2" /> {loading ? "Processando..." : "Publicar Mudanças"}
        </Button>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={onTabChange} className="space-y-8">
        <div className="flex flex-col items-center sm:items-start gap-4">
           <TabsList className="bg-muted p-1 rounded-xl gap-1 flex-wrap max-w-full justify-start overflow-x-auto">
            {(isSuperAdmin || hasPermission('aparencia_visual')) && (
                <TabsTrigger value="visual" className="rounded-lg px-4 py-2.5 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm font-semibold text-sm transition-all whitespace-nowrap">
                  <Palette className="w-4 h-4 mr-2" /> Identidade Visual
                </TabsTrigger>
            )}
            {(isSuperAdmin || hasPermission('aparencia_textos')) && (
                <TabsTrigger value="textos" className="rounded-lg px-4 py-2.5 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm font-semibold text-sm transition-all whitespace-nowrap">
                  <Type className="w-4 h-4 mr-2" /> Texto & Cores
                </TabsTrigger>
            )}
            {(isSuperAdmin || hasPermission('aparencia_menus')) && (
                <TabsTrigger value="menus" className="rounded-lg px-4 py-2.5 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm font-semibold text-sm transition-all whitespace-nowrap">
                  <ExternalLink className="w-4 h-4 mr-2" /> Menu do Site
                </TabsTrigger>
            )}
            {(isSuperAdmin || hasPermission('aparencia_layout')) && (
                <TabsTrigger value="layout" className="rounded-lg px-4 py-2.5 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm font-semibold text-sm transition-all whitespace-nowrap">
                  <Layout className="w-4 h-4 mr-2" /> Design do Site
                </TabsTrigger>
            )}
            {(isSuperAdmin || hasPermission('visibilidade')) && (
                <TabsTrigger value="visibilidade" className="rounded-lg px-4 py-2.5 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm font-semibold text-sm transition-all whitespace-nowrap">
                  <Eye className="w-4 h-4 mr-2" /> Visibilidade
                </TabsTrigger>
            )}
            {isSuperAdmin && (
                <TabsTrigger value="whitelabel" className="rounded-lg px-4 py-2.5 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm font-semibold text-sm transition-all whitespace-nowrap">
        ...
        <TabsContent value="visual" className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-8">
                 <Card className="rounded-xl border border-border shadow-sm overflow-hidden bg-card text-card-foreground">
                    <CardHeader className="p-6 border-b border-border/50">
                      <CardTitle className="text-base font-bold text-foreground">Botão Play (Ao Vivo)</CardTitle>
                    </CardHeader>
                 ...
                 <Card className="rounded-xl border border-border shadow-sm overflow-hidden bg-card text-card-foreground">
                    <CardHeader className="p-6 border-b border-border/50">
                      <CardTitle className="text-base font-bold text-foreground">Navegação (Tabs)</CardTitle>
                    </CardHeader>
            ...
            <Card className="rounded-xl border border-border shadow-sm bg-card text-card-foreground overflow-hidden">
              <CardHeader className="p-6 border-b border-border/50">
                <CardTitle className="text-base font-bold text-foreground flex items-center gap-2">
                  <Palette className="w-5 h-5 text-primary" /> Cores de Texto & Descrição
                </CardTitle>
              </CardHeader>
        ...
        <TabsContent value="menus" className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
          <Card className="rounded-xl border border-border shadow-sm overflow-hidden bg-card text-card-foreground">
            <CardHeader className="p-6 border-b border-border/50">
              <CardTitle className="text-base font-bold text-foreground">Links de Navegação</CardTitle>
              <CardDescription className="text-sm">Personalize o menu principal superior</CardDescription>
            </CardHeader>
        ...
        <TabsContent value="layout" className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <Card className="rounded-xl border border-border shadow-sm bg-card text-card-foreground">
                 <CardHeader className="p-6 border-b border-border/50">
                    <CardTitle className="text-base font-bold text-foreground">Cidade Padrão</CardTitle>
                    <CardDescription className="text-sm">Define a localização base exibida no site</CardDescription>
                 </CardHeader>
              ...
              <Card className="rounded-xl border border-border shadow-sm bg-card text-card-foreground">
                 <CardHeader className="p-6 border-b border-border/50">
                    <CardTitle className="text-base font-bold text-foreground">Cores do Plantão</CardTitle>
                 </CardHeader>
        ...
        <TabsContent value="visibilidade" className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
           <Card className="rounded-xl border border-border shadow-sm bg-card text-card-foreground overflow-hidden">
              <CardHeader className="p-6 border-b border-border/50">
                 <CardTitle className="text-base font-bold text-foreground">Modularidade do Site</CardTitle>
                 <CardDescription className="text-sm mt-1">Ative ou oculte seções completas do site</CardDescription>
              </CardHeader>
           ...
           {/* SEÇÃO MANUTENÇÃO */}
           <Card className="rounded-xl border border-border shadow-sm bg-card text-card-foreground overflow-hidden mt-8">
              <CardHeader className="p-6 border-b border-border/50">
                 <CardTitle className="text-base font-bold text-red-500 flex items-center gap-2">
                   <Settings className="w-5 h-5 animate-spin" /> Modo Manutenção
                 </CardTitle>
                 <CardDescription className="text-sm mt-1">Tire o site do ar para os ouvintes, mantendo apenas o admin acessível</CardDescription>
              </CardHeader>
        ...
        {isSuperAdmin && (
          <TabsContent value="whitelabel" className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
            <Card className="rounded-xl border border-border shadow-sm bg-card text-card-foreground overflow-hidden">
               <CardHeader className="p-6 border-b border-border/50">
                 <CardTitle className="text-base font-bold text-foreground">Personalização de Marca (Painel)</CardTitle>
                 <CardDescription className="text-sm mt-1">Configure as cores do ambiente administrativo</CardDescription>
               </CardHeader>
