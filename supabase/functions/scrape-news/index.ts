...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: `O site ${new URL(formattedUrl).hostname} recusou a conexão (Erro ${response.status}). Alguns portais de notícias bloqueiam sistemas automáticos.` }),
